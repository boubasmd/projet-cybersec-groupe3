...
if (!$offre) {
    $_SESSION['erreur'] = "Cette offre n'existe pas.";
    header('Location: index.php');
    exit;
}

if ($offre['id_utilisateurs'] == $id_user) {
    $_SESSION['erreur'] = "Vous ne pouvez pas postuler à votre propre offre.";
...
if ($stmt2->fetch()) {
    $_SESSION['erreur'] = "Vous avez déjà postulé à cette offre.";
    header("Location: offre.php?id=$id_offre");
    exit;
}
...
$_SESSION['succes'] = "Votre candidature a bien été envoyée !";
header("Location: offre.php?id=$id_offre");
exit;
